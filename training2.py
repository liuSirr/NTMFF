...
for dataset in datasets:
    ...
    if ((not os.path.isfile(processed_data_file_train)) or (not os.path.isfile(processed_data_file_test))):
        ...
        best_ci = 0
        best_epoch = -1
        model_file_name = 'model_' + model_st + '_' + dataset + '_with_all' + '.model'
        result_file_name = 'result_' + model_st + '_' + dataset + '_with_all' + '.csv'
        logging.basicConfig(filename='log_' + model_st + '_' + dataset + '_with_all' + '.log', level=logging.INFO,
        ...
        for epoch in range(NUM_EPOCHS):
            ...
            if ret[1] < best_mse:
                ...
                print('rmse improved at epoch ', best_epoch, '; best_mse,best_ci:', best_mse, best_ci, model_st,
                      dataset)
                # variable = 42
                logging.info(f'rmse improved at epoch {best_epoch}; best_mse,best_ci:{best_mse, best_ci, model_st, dataset}')
            else:
                print(ret[1], 'No improvement since epoch ', best_epoch, '; best_mse,best_ci:', best_mse, best_ci,
                      model_st, dataset)
                logging.info(f'{ret[1]}, No improvement since epoch  {best_epoch}; best_mse,best_ci:{best_mse, best_ci, model_st, dataset}')

        # 如果需要记录更详细的信息，可以使用不同的级别，如debug、warning、error等
