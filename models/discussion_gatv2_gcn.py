...
class discussion_GATv2_GCN(torch.nn.Module):
    def __init__(self, n_output=1, num_features_xd=78, num_features_xt=25,
                 n_filters=32, embed_dim=128, output_dim=128, dropout=0.2,
                 ss_feature=8, sas_feature=3, map_feature=2):

        super(discussion_GATv2_GCN, self).__init__()
    ...
    def forward(self, data):
        ...
        # ss_feat
        temp_batch = torch.LongTensor(np.ones(ss_feat.size(0)) * (2-1)).to("cuda:0")
        ss_feat = torch.cat([gmp(ss_feat, temp_batch), gap(ss_feat, temp_batch)], dim=1)
        ss_feat = self.fc_g3(ss_feat)

        # sas_feat
        temp_batch = torch.LongTensor(np.ones(sas_feat.size(0)) * (2 - 1)).to("cuda:0")
        sas_feat = torch.cat([gmp(sas_feat, temp_batch), gap(sas_feat, temp_batch)], dim=1)
        sas_feat = self.fc_g4(sas_feat)

        # eds_contact
        temp_batch = torch.LongTensor(np.ones(eds_contact.size(0)) * (2 - 1)).to("cuda:0")
